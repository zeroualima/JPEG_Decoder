...
// SOF0
void write_SOF0(FILE *f, uint16_t hauteur, uint16_t largeur, uint8_t nb_couleurs, uint8_t fH_Y, uint8_t fV_Y, uint8_t fH_Cb, uint8_t fV_Cb,  uint8_t fH_Cr, uint8_t fV_Cr) {
...
void write_DHT(FILE *f, uint8_t is_AC, uint8_t iH, const uint8_t nb_symb_per_lengths[16], const uint8_t *symboles) {
    ...
    uint8_t entete[] = {
        ...
        (longueur >> 8) & 0xFF, longueur & 0xFF, // longueur big-endian
        (is_AC << 4) | iH, // type (0=DC, 1=AC) + indice iH = 0...3
    };
    fwrite(entete, 1, sizeof(entete), f);

    // Les 16 nombres de symboles par profondeur dans la table de Huffmann
    fwrite(nb_symb_per_lengths, 1, 16, f);

    // les N symboles
    fwrite(symboles, 1, N, f);
}

// SOS
void write_SOS(FILE *f, uint8_t nb_couleurs, uint8_t iH_DC_Y, uint8_t iH_AC_Y, uint8_t iH_DC_C, uint8_t iH_AC_C) {

    // longueur = 2 + 1 + 2*nb_couleurs + 3
    ...
    if (nb_couleurs == 1) {
        // niveaux de gris : une seule composante Y
        uint8_t comp[] = { 0x01, (iH_DC_Y << 4) | iH_AC_Y };
        fwrite(comp, 1, sizeof(comp), f);
    } else {
    ...
    // baseline : toujours ces 3 octets
    uint8_t fin[] = { 0x00, 0x3F, 0x00 };
...
void add_JPEG_entete(FILE *f, uint16_t hauteur, uint16_t largeur, uint8_t nb_couleurs, sampling_factors s) {
    ...
    // SOF0
    write_SOF0(f, hauteur, largeur, nb_couleurs, fH_Y, fV_Y, fH_Cb, fV_Cb, fH_Cr, fV_Cr);
